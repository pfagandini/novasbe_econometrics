...
$$\text{se}(\hat{\beta}_1) = \frac{\hat{\sigma}}{\sqrt{S_{xx}}} \qquad \text{se}(\hat{\beta}_0) = \hat{\sigma}\sqrt{\frac{1}{n} + \frac{\bar{x}^2}{S_{xx}}}$$

```{r}
se_beta1 <- sigma_hat / sqrt(Sxx)
se_beta0 <- sigma_hat * sqrt(1/n + x_bar^2 / Sxx)

cat("se(β̂₀) =", round(se_beta0, 4), "\n")
...
cat(sprintf("β̂₀  %10.4f  %10.4f  %8.3f   %.2e\n",
            beta0_hat, se_beta0, t_beta0, p_beta0))
cat(sprintf("β̂₁  %10.4f  %10.4f  %8.3f   %.2e\n",
...
ci_beta0 <- beta0_hat + c(-1, 1) * t_crit * se_beta0
ci_beta1 <- beta1_hat + c(-1, 1) * t_crit * se_beta1

cat("95% CI for β₀: [", round(ci_beta0[1], 4), ",", round(ci_beta0[2], 4), "]\n")
...
## 20 different samples → 20 different lines
